...
const LoginPage = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.message) {
          ...
          if (response.message.toLowerCase().includes("email") || response.message.toLowerCase().includes("user not found")) {
            setErrors(prev => ({...prev, email: response.message, password: ''}));
          } else if (response.message.toLowerCase().includes("password")) {
            setErrors(prev => ({...prev, password: response.message, email: ''}));
          } else {
  ...
  const handleGoogleLogin = () => {
    toast.info('Google Sign-In not implemented in this example.');
  };
    ...
    <>
        ...
        <div className="min-h-screen flex flex-col md:flex-row items-stretch relative z-10">
          ...
          >
            <motion.div
              ...
              className="z-10 cheapal-image-logo-wrapper"
            ...
            <motion.p
              className="text-xl lg:text-2xl text-[var(--misty-gray)] text-center max-w-md mt-8 mb-8 z-10"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.4 }}
            >
              Access your account for exclusive subscription deals.
            </motion.p>
            <div className="mt-auto mb-24 text-[var(--misty-gray)] opacity-70 text-xs z-10">
              © {new Date().getFullYear()} Cheapal Inc. All rights reserved.
            </div>
          </motion.div>

          <div className="md:w-3/5 lg:w-1/2 w-full flex items-center justify-center p-6 sm:p-8 md:p-12 relative">
            <motion.main
              ...
              animate="visible"
            >
              <div className="classified-stamp">SECURE LOGIN</div>
              <div className="form-container" id="login-container-form">
                <motion.h1
                  data-text="Account Authentication"
                  ...
                  <motion.div className="form-group" variants={formItemVariants}>
                    ...
                    <div className="input-container">
                      ...
                      <input
                        ...
                        placeholder="secure.transmission@domain.net"
                        className={errors.email && errors.email !== ' ' ? 'input-error' : ''}
                      />
                      <div className="input-glow"></div>
                    </div>
                    {errors.email && errors.email !== ' ' && <motion.p variants={errorVariants} initial="hidden" animate="visible" className="form-error-text">{errors.email}</motion.p>}
                  </motion.div>
                  <motion.div className="form-group" variants={formItemVariants}>
                    ...
                    {errors.password && errors.password !== ' ' && <motion.p variants={errorVariants} initial="hidden" animate="visible" className="form-error-text">{errors.password}</motion.p>}
                  </motion.div>
                ...
                <motion.div className="form-footer mt-6" variants={formItemVariants}>
                  <p>
                    <Link to="/forgot-password" className="accent-link text-xs">Password Recovery</Link>
                  </p>
                  <p className="mt-2">New User? <Link to="/register" className="accent-link">Request Access</Link></p>
                </motion.div>
              </div>
            </motion.main>
          </div>
      ...
      <style jsx global>{`
        ...
        .cheapal-image-logo-wrapper img {
          filter: drop-shadow(0 0 10px rgba(var(--bioluminescent-teal-rgb), 0.3))
                  drop-shadow(0 0 20px rgba(var(--bioluminescent-teal-rgb), 0.2));
          transition: filter 0.3s ease-in-out;
        }

        .cheapal-image-logo-wrapper img:hover {
          filter: drop-shadow(0 0 15px rgba(var(--bioluminescent-teal-rgb), 0.5))
                  drop-shadow(0 0 30px rgba(var(--bioluminescent-teal-rgb), 0.3));
        }
