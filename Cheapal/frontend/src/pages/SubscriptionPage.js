...
const ListingCard = ({ listing, seller }) => {
    ...
    console.log("[ListingCard] Listing ID:", listing._id);
    console.log("[ListingCard] Listing mainRank:", listing.mainRank);
    console.log("[ListingCard] Seller ID:", seller?._id);
    console.log("[ListingCard] Seller data:", seller);
    console.log("[ListingCard] Seller badges:", seller?.badges || "No badges");
            ...
            >
                <div className="p-5 flex flex-col flex-grow">
                    ...
                    <div className="h-52 bg-gray-700/50 relative overflow-hidden rounded-lg mb-0">
                        <img
                            src={listing.image ? `${IMAGE_BASE_URL}/Uploads/${listing.image}` : 'https://placehold.co/600x400/2D3748/A0AEC0?text=Listing'}
                        ...
                        <div className="w-9 h-9 rounded-full bg-gray-600 flex items-center justify-center overflow-hidden text-sm font-bold text-white flex-shrink-0 ring-1 ring-gray-500">
                            {seller?.avatar ? (
                                <img
                                    ...
                                    onError={(e) => { e.target.onerror = null; e.target.outerHTML = `<span className="text-xs" title="${seller.name || ''}">${getInitials(seller.name)}</span>`; }}
                                />
                            ) : (
...
const SubscriptionPage = () => {
    ...
    const fetchListings = useCallback(async () => {
        ...
        setError(null);
        try {
            const approvedListingsResponse = await listingService.getApproved();
            console.log("[SubscriptionPage] Approved listings response:", JSON.stringify(approvedListingsResponse, null, 2));
            
            let approvedListings = [];
            if (approvedListingsResponse && approvedListingsResponse.success && Array.isArray(approvedListingsResponse.data)) {
                approvedListings = approvedListingsResponse.data;
            } else if (Array.isArray(approvedListingsResponse)) {
                approvedListings = approvedListingsResponse;
            } else {
                console.error("[SubscriptionPage] listingService.getApproved did not return an array or valid response:", approvedListingsResponse);
                setListings([]);
                setError("Received invalid data format for listings.");
                setLoading(false);
                return;
            }

            // Sort listings by mainRank (ascending, null last) and createdAt (descending)
            approvedListings.sort((a, b) => {
                const rankA = a.mainRank != null ? a.mainRank : Number.MAX_SAFE_INTEGER;
                const rankB = b.mainRank != null ? b.mainRank : Number.MAX_SAFE_INTEGER;
                if (rankA !== rankB) return rankA - rankB;
                return new Date(b.createdAt) - new Date(a.createdAt);
            });

            console.log("[SubscriptionPage] Approved listings count:", approvedListings.length);
            console.log("[SubscriptionPage] Approved listings order:", approvedListings.map(l => ({ _id: l._id, mainRank: l.mainRank })));
            setListings(approvedListings);

            if (approvedListings.length === 0) {
                console.warn("[SubscriptionPage] No approved listings found.");
            }

            // Fetch seller data for each unique sellerId
            const sellerIds = [...new Set(approvedListings.map(l => l.sellerId?._id).filter(id => id))];
            const sellerPromises = sellerIds.map(async (sellerId) => {
                try {
                    ...
                    console.warn(`[SubscriptionPage] No seller data for ID: ${sellerId}`);
                    return { [sellerId]: null };
                } catch (err) {
                    console.error(`[SubscriptionPage] Error fetching seller ${sellerId}:`, err);
                    return { [sellerId]: null };
                }
            });
            ...
            console.error("[SubscriptionPage] Error in fetchListings:", err);
                ...
                <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow">
                    <div className="mb-10 pt-8 text-center">
                        ...
                        <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto">
                            Discover a wide range of digital services and subscriptions tailored for you.
                        </p>
                    </div>
            ...
            <style jsx global>{`
                ...
                .golden-btn {
                    ...
                    box-shadow: inset 0 0 0 1px #A2790D,
                                ...
                                inset 0 0.125em lighten(#FFD700, 15%),
